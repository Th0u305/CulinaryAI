...
import { Clock, ChefHat, Bookmark, ArrowUp, Heart } from "lucide-react";
...
const RecipeCardData = () => {
  ...
  const handleSaveButton = async (id: string) => {
    ...
    await fetch("/api/updateUser/save", {
      ...
      .then((data) => {
        if (data.error) {
          return toast.error("You already saved this recipe");
        }
          ...
          >
            <div className="relative aspect-video overflow-hidden">
              <Button
                ...
                variant="ghost"
                size="sm"
                className="bg-red-100 absolute z-20 right-2 top-2 button-color hover:bg-red-100"
              >
